...
import { isValidModule, getOrduresConfig } from '@ordures/shared-utils';
...
export default class Plugin {
  constructor(
    ...
    this.polyfillPath = polyfillPath;
  }
  ...
  importOrduresUiPolyfill(file, transformedMethodName) {
    const { polyfillPath } = this;
    if (!!polyfillPath && isValidModule(transformedMethodName)) {
      const { theme, directory, enabled } = getOrduresConfig();
      if (!enabled) return;
      const polyfill = join(cwd(), `./node_modules/${polyfillPath}`); // /**/node_modules/@ordures-ui
      const isOrduresInstalled = existsSync(polyfill);
      if (!isOrduresInstalled) throw new Error(`未安装依赖： ${polyfillPath}`);
      const modulePath = winPath(join(polyfillPath, directory, theme, transformedMethodName, 'index.js')); // @ordures-ui/es/default/button/index
      const existModule = existsSync(join(cwd(), './node_modules/', modulePath));
      if (existModule) {
        addSideEffect(file.path, `${modulePath}`);
      }
    }
  }
  ...
  importMethod(methodName, file, pluginState) {
    if (!pluginState.selectedMethods[methodName]) {
      ...
      this.importOrduresUiPolyfill(file, transformedMethodName);
